...
public class Flashcart_wishlist_mycartTest extends configurationpage {
	@Test
	public void xyz() throws InterruptedException
	{
		WishlistPageUserPage wup = new WishlistPageUserPage(driver);
		...
		System.out.println(cpu.pageVerification());
		...
		Assert.assertEquals(driver.findElement(By.xpath("//ul/li[1]")).getText(), "Welcome -Anuj Kumar");
		...
		cpu.searchAction(xlu.getData(Excelsheet.CONTACTS.getSheetName(),Excelsheet.WISHLISTMYCART.getSheetName(), "Search"));
		sr.addToWishlisttbtnclk();
		Assert.assertEquals((wup.pageVerification()),"MY WISHLIST");
		wup.addtoCartbtn();
		cpu.clickOnCommonTabInUpperNvgbtn(CommonTabNamesUser.MYCART.getTabName());
		Assert.assertEquals(scup.pageVerification(),"Shopping Cart");
